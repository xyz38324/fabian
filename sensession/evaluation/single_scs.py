...
def scale_magnitude(df: pl.DataFrame) -> pl.DataFrame:
    ...
    return df.join(
        # We filter out the modified and its neighboring subcarriers to not have them affect our
...
def get_masked_data(experiment_name: str):
...
def plot_single_idx(idx: int, df: pl.DataFrame, outdir: Path):
...
def plot_all_shapes(df: pl.DataFrame, outdir: Path):
    for modified_idx, group in df.group_by(pl.col("modified_idx")):
        ...
        plot_single_idx(modified_idx, group, outdir)


def plot_observed_scalings(df: pl.DataFrame, outdir: Path):
    num_receiver = df.select(pl.col("receiver_name")).n_unique()
    num_used_antennas = df.select(pl.col("used_antenna_idxs")).n_unique()

    # We plot only detected changes on the subcarrier we precoded
    df = df.filter(pl.col("modified_idx") == pl.col("subcarrier_idxs"))
    ...
    f.suptitle("CSI scaling detection on different subcarriers")
    f.savefig(outdir / f"factor_detection_plot.png", dpi=f.dpi)
    plt.close(f)


if __name__ == "__main__":
    experiment_name = "single_sc_mods"
    outdir = Path.cwd() / "data" / experiment_name / "images"
    outdir.mkdir(parents=True, exist_ok=True)
    data = get_masked_data(experiment_name)

    plot_all_shapes(data, outdir)

    plot_observed_scalings(data, outdir)
