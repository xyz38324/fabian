...
def get_masked_data(exp_name: str):
    ...
    if cache_file.is_file():
        logger.debug("Data already preprocessed. Loading cached file ...")
        data = pl.read_parquet(cache_file)
        logger.debug(f"Loaded data of shape: {data.shape}")
        return data

    db_path = Path.cwd() / "data" / exp_name / "db.parquet"
    logger.debug(f"Loading data from database {db_path}")
    if not db_path.is_file():
        logger.debug(f"Path {db_path} does not point to a file")
