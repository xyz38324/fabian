...
def scale_magnitude(df: pl.DataFrame) -> pl.DataFrame:
    ...
    return df.join(
        ...
        df.filter(
            (pl.col("subcarrier_idxs") > pl.col("modified_idx") + 2)
            | (pl.col("subcarrier_idxs") < pl.col("modified_idx") - 2)
        )
        .group_by("row_nr")
...
def plot_single_mask_amplitudes(idx: int, df: pl.DataFrame, outdir: Path):
    ...
    logger.trace(
        "Sanity check. Plotting single mask amplitudes "
        f" -- dataframe shape  : {df.shape}\n"
        f" -- output directory : {outdir}\n"
        f" -- Modified index   : {idx}\n"
    )
    ...
    for (i, j), ((receiver_name, antenna), data) in zip(subplt_idxs, group_df):
        ...
        factors = data.get_column("factor").unique(maintain_order=True).to_list()
        labels = [
            f"{round(factor,2)} π = {round(factor * np.pi, 2)}" for factor in factors
        ]
        lineplot(
            data=data,
...
def plot_single_mask_phases(
    idx: int, receiver_name: str, df: pl.DataFrame, outdir: Path
):
    """
    ...
    logger.trace(
        "Sanity check. Plotting phases detected for introducing phase jump on single"
        f" idx.  -- dataframe shape  : {df.shape}\n -- output directory : {outdir}\n --"
        f" Modified index   : {idx}\n"
    )

    group_df = df.sort("factor").group_by("factor", maintain_order=True)
    ...
    f, axes = plt.subplots(
        num_factors,
        1,
        figsize=(10, 6 * num_factors),
        sharex=True,
        squeeze=False,
    )
    plt.subplots_adjust(right=0.75)

    subplt_idxs = range(num_factors)

    for i, (factor, data) in zip(subplt_idxs, group_df):
        ax = axes[i, 0]
...
def plot_single_mask_phase_corrected(
    idx: int, receiver_name: str, df: pl.DataFrame, outdir: Path
):
    """
...
def plot_all_shapes(df: pl.DataFrame, outdir: Path):
    ...
    for (modified_idx, receiver_name), group in df.group_by(
        "modified_idx", "receiver_name"
    ):
        logger.trace(
            "Plotting phases for phase shift introduced on subcarrier index"
            f" {modified_idx}"
        )
        recv_outdir = outdir / receiver_name
        recv_outdir.mkdir(exist_ok=True, parents=True)
        plot_single_mask_phases(modified_idx, receiver_name, group, recv_outdir)
        plot_single_mask_phase_corrected(
            modified_idx, receiver_name, group, recv_outdir
        )

    for modified_idx, group in df.group_by(pl.col("modified_idx")):
        logger.trace(
            "Plotting CSI magnitude for session with phase shift on subcarrier index"
            f" {modified_idx}"
        )
        plot_single_mask_amplitudes(modified_idx, group, outdir)


def plot_observed_jumps(df: pl.DataFrame, outdir: Path):
    num_receiver = df.select(pl.col("receiver_name")).n_unique()
    num_used_antennas = df.select(pl.col("used_antenna_idxs")).n_unique()

    # Filter out all phase values at the phase jump position
    jump_df = df.filter(pl.col("subcarrier_idxs") == pl.col("modified_idx")).rename({
        "normed_csi_phase": "post_phase",
        "shape_normed_csi_phase": "post_shape_normed_phase",
    })

    # Filter out all phase values just before the jump position
    pre_jump_df = df.filter(
        pl.col("subcarrier_idxs") == pl.col("modified_idx") - 1
    ).rename({
        "normed_csi_phase": "pre_phase",
        "shape_normed_csi_phase": "pre_shape_normed_phase",
    })

    # Join them together and calculate the difference to estimate the
    # phase jump!
    filtered_df = jump_df.join(
        pre_jump_df, on=["receiver_name", "sequence_number", "timed_session_id"]
    ).with_columns(
        phase_jump=pl.col("post_phase").sub(pl.col("pre_phase")),
        normed_phase_jump=pl.col("post_shape_normed_phase").sub(
            pl.col("pre_shape_normed_phase")
        ),
    )

    # Group into subplots
    subplt_idxs = product(range(num_receiver), range(num_used_antennas))
    group_df = filtered_df.sort("receiver_name").group_by(
        "receiver_name", "used_antenna_idxs", maintain_order=True
    )
    ...
    f.suptitle("Detected phase jump (uncorrected)")
    f.savefig(outdir / f"uncorrected_phasejump_detection.png", dpi=f.dpi)
    plt.close(f)
    ...
    f.suptitle("Detected phase jump (after shape correction)")
    f.savefig(outdir / f"corrected_phasejump_detection.png", dpi=f.dpi)
    plt.close(f)


if __name__ == "__main__":
    exp_name = "phase_jumps"
    outdir = Path.cwd() / "data" / exp_name / "images"
    outdir.mkdir(parents=True, exist_ok=True)
    data = get_masked_data(exp_name)

    plot_observed_jumps(data, outdir)
