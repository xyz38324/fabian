...
def triangle_iwl_session(sc_idx, scale):
...
def triangle_qca_session(sc_idx, scale):
    mask = triangle_mask(sc_idx, scale)
...
def triangle_exp_config() -> ExperimentConfig:
    ...
    iwl_sessions = [
        triangle_iwl_session(sc_idx, scale)
        for sc_idx, scale in itertools.product(idxs, factors)
    ]

    qca_sessions = [
        triangle_qca_session(sc_idx, scale)
        for sc_idx, scale in itertools.product(idxs, factors)
    ]

    sessions = iwl_sessions + qca_sessions

    return exp_config(exp_id="triangle_precoding", sessions=sessions)


if __name__ == "__main__":
    run_exp(experiment_cfg=triangle_exp_config())
